...
public class PickFactory {
	...
	public static void update(float screenX, float screenY) {
		for(int i=0;i<16;i++){
			Log.i("sceneState.gMatView"+String.valueOf(i), String.valueOf(sceneState.gMatView.asFloatBuffer().get(i)));
		}

		for(int i=0;i<16;i++){
			Log.i("sceneState.gMatProject"+String.valueOf(i), String.valueOf(sceneState.gMatProject.asFloatBuffer().get(i)));
		}
		for(int i=0;i<16;i++){
			Log.i("sceneState.gpMatrixViewArray",String.valueOf(sceneState.gpMatrixViewArray[i]) );
		}
		...
		for(int i=0;i<16;i++){
			Log.i("sceneState.gpMatrixViewArray after fillFloatArray",String.valueOf(sceneState.gpMatrixViewArray[i]) );
		}
		...
		gProjector.gluUnProject(screenX, openglY, 0.0f,
				sceneState.gpMatrixViewArray, 0, sceneState.gpMatrixProjectArray,
				0, sceneState.gpViewport, 0, gpObjPosArray, 0);
		// 填充射线原点P0
