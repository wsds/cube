...
import com.cube.common.pickup.Vector4f;
			...
			for (int j = 0; j < 2; j++) {
				...
				if (ray.intersectTriangle(v0, v1, v2, location)) {
					...
					if (!bFound) {
						...
						bFound = true;
						closeDis = location.w;
						trianglePosOut[0].set(getVector3f(indices[i * 4 + 0]));
						trianglePosOut[1].set(getVector3f(indices[i * 4 + 1]));
						trianglePosOut[2].set(getVector3f(indices[i * 4 + 2]));
						trianglePosOut[3].set(getVector3f(indices[i * 4 + 3]));
						sceneState.picked = i;
						surface = i;
					} else {
