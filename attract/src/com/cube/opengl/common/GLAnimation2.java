...
public class GLAnimation2 {
	ArrayList<GLAnimation2> next = new ArrayList<GLAnimation2>();
	ArrayList<GLAnimation2> children = null;
	...
	// initialize method
	public void addNextAnimation(GLAnimation2 animation) {
	...
	public void setScale(float ds, float fx, float fy, float fz, float dt) {
		this.scale.ds = ds;
		this.dt = dt;
		this.scale.fx = fx;
		this.scale.fy = fy;
	...
	public boolean transformModel(GlMatrix matrix) {
		...
		if (remainTime == 0) {
			...
			if (lastMillis != 0 && remainTime > 0) {
				...
				if (this.type == TRANSLATE) {
					matrix.translate(translate.dx / dt * delta, translate.dy / dt * delta, translate.dz / dt * delta);
				} else if (this.type == ROTATE) {
					GlMatrix rotateMatrix = new GlMatrix();
					rotateMatrix.rotate(rotate.dr / dt * delta, rotate.fx, rotate.fy, rotate.fz);
					matrix.multiply(rotateMatrix);
				} else if (this.type == SCALE) {
					// matrix.scale(1 + delta * (scale.ds - 1) / dt, 1 + delta * (scale.ds - 1) / dt, scale.fx, scale.fy, scale.fz);
